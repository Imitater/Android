...
public class VideoRvAdapter extends RecyclerView.Adapter<VideoRvAdapter.MyViewHolder> {
    ...
    public VideoRvAdapter(Context context, List<ClassBean> list){
        this.context = context;
        this.list = list;
    }
    @Override
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new MyViewHolder(LayoutInflater.from(context).inflate(R.layout.item_video,null));
    }
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        Picasso.with(context).load(AppConstants.IMG_BASE_URL+bean.getBgimg()).fit().placeholder(R.mipmap.good_default).error(R.mipmap.good_default).into(holder.iv_item_bg);
