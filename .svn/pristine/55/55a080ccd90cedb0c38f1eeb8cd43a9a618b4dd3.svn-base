package com.ruiyihong.toyshop.util.videoCompressor;
...
public class MediaController {
    ...
    public boolean convertVideo(final String path) {
        ...
        if (TextUtils.isEmpty(comPath)){
            Log.i("radish","path------------------null" );
            //默认路径
            comPath = Environment.getExternalStorageDirectory()
                    + File.separator
                    + AppConstants.VIDEO_COMPRESSOR_APPLICATION_DIR_NAME
                    + AppConstants.VIDEO_COMPRESSOR_COMPRESSED_VIDEOS_DIR + File.separator +
                    "VIDEO_" + new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(new Date()) + ".mp4";
        }
        File cacheFile = new File(comPath);
